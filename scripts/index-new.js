...
// Animation - First Page Load (Modern Bento Capsule Loader)
function initLoaderHome() { 
  ...
  var numEl = document.querySelector(".loader-number");
  ...
  tl.to(counterObj, {
    ...
    ease: "power2.inOut",
    onUpdate: function() {
      var current = Math.floor(counterObj.val);
      if (numEl) numEl.textContent = current;
      if (statusEl) {
        if (current > 75) {
          statusEl.textContent = "System Ready";
        } else if (current > 35) {
          statusEl.textContent = "Loading Web Environment";
        }
      }
    }
  }, "=-0.2");
...
function initScrollLetters() {
  ...
  const roll1 = roll(".hero-ticker-bar .ticker-content", {duration: 25}),
  ...
  function roll(targets, vars, reverse) {
    ...
    const tl = gsap.timeline({
          ...
          positionClones = () => elements.forEach((el, i) => gsap.set(clones[i], {position: "absolute", overwrite: false, top: el.offsetTop || 0, left: el.offsetLeft + (reverse ? -el.offsetWidth : el.offsetWidth)}));
    positionClones();
